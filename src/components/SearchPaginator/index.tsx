...
const SearchPaginator = ({
  totalCount, currentOffset, onPageChange, itemsPerPage = 12,
}: PagerProps) => {
  const totalPageCount = Math.ceil(totalCount / itemsPerPage);
  const [index, setIndex] = useState(currentOffset / itemsPerPage);

  useEffect(() => {
    setIndex(currentOffset / itemsPerPage);
  ...
  const handleClick = (data: { selected: number }) => {
    onPageChange(data.selected * itemsPerPage);
  };
